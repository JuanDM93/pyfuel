...
class Algo(Process):
    def __init__(
            self, threadID, name, size, vol, radios
    ):
    ...
    def getTemp(self):
        ...
        while finished:
            ...
            if self.cuenta % 10 is 0:
                print 'Cuenta %s: %s %s' % (self.cuenta, delta_max, time.clock())

            # TODO or cuenta is 100, 1000...? deltas
            if (delta_max < converge) or (self.cuenta > self.limit):
        ...
        self.status = 'Prom %f' % calor
        self.third = True

    def setVal(self):
        return 100 * self.cuenta / self.limit
